...
class Exporter:
    ...
    def _export_pylock_toml(self, out_dir: Path) -> str:
        ...
        if self._poetry.package.python_versions != "*":
            python_constraint = self._poetry.package.python_constraint
            if (
                isinstance(python_constraint, VersionRange)
                and python_constraint.max is None
            ):
                ...
                if isinstance(python_constraint, VersionRange):
                    lock["requires-python"] = f">={python_constraint.min}"
        ...
        for dependency_package in get_project_dependency_packages2(
            ...
            match dependency:
                ...
                case FileDependency():
                    file_: dict[str, Any] = {}
                    try:
                        file_["path"] = dependency.full_path.relative_to(
                            out_dir
                        ).as_posix()
                    except ValueError:
                        file_["path"] = dependency.full_path.as_posix()
                    assert len(package.files) == 1, (
                        "FileDependency must have exactly one file"
                    )
                    file_["hashes"] = dict([package.files[0]["hash"].split(":", 1)])
                    if dependency.directory:
                        file_["subdirectory"] = dependency.directory
                    data["archive"] = file_
                case URLDependency():
                    url_: dict[str, Any] = {}
                    url_["url"] = dependency.url
                    assert len(package.files) == 1, (
                        "URLDependency must have exactly one file"
                    )
                    url_["hashes"] = dict([package.files[0]["hash"].split(":", 1)])
                    if dependency.directory:
                        url_["subdirectory"] = dependency.directory
                    data["archive"] = url_
                case _:
